...
void loop() {
  ...
  sensorXYZ local_accels,world_accels,vels;
  quaternions qua;
  ...
  if(flag) vels = get_velocities(global_accX,global_accY,global_accZ);//最初、z軸加速度にずれが生じるので0付近になるまで待つ
  else if(!flag&&global_accZ<0.01){
    flag = true;
  }

  Serial.print(global_accX,2);//出力,シリアルプロッタで閲覧可能
  Serial.print(", ");
  Serial.print(global_accY,2);
  Serial.print(", ");
  Serial.print(global_accZ,2);
  Serial.println();
  delay(dt*1000);
}